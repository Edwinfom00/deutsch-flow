      ...
      <div className="bg-gray-50 border border-gray-200 rounded-md p-4 space-y-3">
        ...
        <div className="flex gap-4 pt-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-500"><span className="font-medium text-gray-700">IA :</span> {exercise.aiRole}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <User className="h-3.5 w-3.5 text-emerald-500" />
            <span className="text-xs text-gray-500"><span className="font-medium text-gray-700">Toi :</span> {exercise.userRole}</span>
          </div>
        </div>
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 space-y-2">
          <textarea
            ...
            disabled={answered}
            placeholder="Réponds en allemand…"
            rows={3}
            className={cn(
          ...
          {phrases.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              ...
              {phrases.map((p, i) => (
                <button key={i} onClick={() => setResponse((prev) => prev + (prev ? " " : "") + p)}
                  disabled={answered}
      ...
      {!answered && (
        <button onClick={handleSubmit} disabled={!response.trim()}
          className={cn(
            ...
            response.trim() ? "bg-gray-900 hover:bg-gray-800 text-white" : "bg-gray-100 text-gray-400 cursor-not-allowed"
          )}>
          Valider ma réponse
        </button>
      )}
